...
const PlaceOrder = () => {
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between gap-6 pt-5 sm:pt-14 min-h-[80vh] border-t">
      {/* Left side */}
      <div className="flex flex-col gap-6 w-full sm:max-w-[600px]">
        <div className="text-xl sm:text-2xl font-semibold my-3">
          <Title text1="DELIVERY" text2=" INFORMATION" />
        </div>
        <div className="flex flex-col sm:flex-row gap-4">
          <input
            className="border border-gray-300 rounded py-2 px-4 w-full  focus:outline-black transition-all"
            type="text"
        ...
        <div className="mt-12">
          ...
          <div className="flex gap-3 flex-col lg:flex-row">
            <div onClick={()=>setMethod('strip')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method==='strip' ? 'bg-blue-400' : ''}`}></p>
          ...
          <div className="w-full text-end mt-8">
            <button onClick={()=>navigate('/orders')} className='bg-black text-white px-16 py-3 text-sm'>PLACE ORDER</button></div> 
        </div>
      </div>
    </div>
  );
